...
dataset_names_k = data.frame(matrix_empty_datasetnames)
...
for(k in 1:5)
{
  ...
  assign(paste("data_k",sampleSize,k,sep="_"), data)
  dataset_names_k = rbind(dataset_names_k, c(paste("data_k",sampleSize,k,sep="_"),k))
...
colnames(dataset_names_k)[1]  <- "names"
colnames(dataset_names_k)[2] <- "values"
